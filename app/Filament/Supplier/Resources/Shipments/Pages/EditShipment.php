...
class EditShipment extends EditRecord
{
    ...
    protected function beforeSave(): void
    {
        ...
        if (! $this->record->isDraft()) {
            ...
            Notification::make()
                ->danger()
                ->title('Cannot Edit')
                ->body('Only draft shipments can be edited.')
                ->send();
        }
    }
    ...
    protected function getHeaderActions(): array
    {
        return [
            ViewAction::make(),
            DeleteAction::make()->visible(fn ($record) => $record->isDraft()),
            ForceDeleteAction::make()->visible(fn ($record) => $record->isDraft()),
            RestoreAction::make()->visible(fn ($record) => $record->isDraft()),
        ];
    }
